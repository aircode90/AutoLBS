...
local function parseNmea(s)
    ...
    elseif smatch(s,"RMC") then
        ...
        if fixed and enGpsPoint then
             local curloc=getLocation()
             if isFix() and ( curloc.lat ~=nil and #curloc.lat >0 ) and ( curloc.lng ~=nil and #curloc.lng >0 )  then 
                point_cnt = point_cnt +1
                log.info("publish GPS_POINT", point_cnt)
                sys.publish("GPS_POINT",{loc=curloc,spd=getSpeed(),cog=getCourse(),altitude =getAltitude(), wxcnt = getUsedSateCnt(), pcnt= point_cnt })
             end
        end
...
--- 设置GPS获得点位报告允许.
-- @bEnabled  开启或者关闭gps点位报告允许
-- @return nil
-- @usage gps.setEnGPSPoint(true)

function setEnGPSPoint(bEnabled)
    enGpsPoint = bEnabled
end
